...
export default class PathsCache extends EventEmitter {
  constructor () {
    super()
    this._projectChangeWatcher = atom.project.onDidChangeFiles(	debounce((events) => {
			if (!events) {
				return;
			}
			events.forEach(({ action, path, oldPath }) => {
				if (action === "renamed" || action === "created" || action === "deleted") {
